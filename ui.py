from tkinter import filedialog, HORIZONTAL, Tk
from student import still_in_development, get_greeting, add_student, update_student_download, delete_student_download, show_data_main, show_data_search, process_file, add_student_details, create_student_pdf, show_data_download, update_student, create_all_student_pdf, delete_student
import pandas as pd
...
def create_main_page_ui(tab, logged_in_user) -> None:
    ...
    database_choice_combobox = ttk.Combobox(search_input_labelframe, textvariable=dat_var, values=dat, state='readonly', bootstyle='SUCCESS')
    ...
    show_button = ttk.Button(search_input_labelframe, text='Show', command=lambda: show_data_main(result_tree1, dat_var, info_label), bootstyle='PRIMARY')
...
def create_search_student_ui(tab, logged_in_user) -> None:
    ...
    search_button = ttk.Button(search_input_labelframe, text='Search', command=lambda: show_data_search(search_entry, result_tree2, dat_var))
    ...
    save_button = ttk.Button(edits_labelframe, text='Save Edit', command=lambda: update_student_info(name_entry, adm_entry, stream_entry, grade_entry, dat_var, result_tree2, search_entry), bootstyle=('SUCCESS', 'OUTLINE'))
    save_button.grid(row=0, column=0, padx=12, pady=5, sticky='ew')

    add_button = ttk.Button(edits_labelframe, text='Add Student', command=lambda: add_student(name_entry.get(), adm_entry.get(), stream_entry.get(), grade_entry.get(), dat_var.get()), bootstyle=('WARNING', 'OUTLINE'))
    add_button.grid(row=0, column=1, padx=12, pady=5, sticky='ew')

    delete_button = ttk.Button(edits_labelframe, text='Delete Student', command=lambda: delete_student_info(result_tree2, dat_var), bootstyle=('DANGER', 'OUTLINE'))
...
def update_student_info(name_entry, adm_entry, stream_entry, grade_entry, dat_var, result_tree, search_entry):
    ...
    name = name_entry.get()
    admission_number = adm_entry.get()
...
def delete_student_info(result_tree, dat_var):
    selected_item = result_tree.selection()
    if not selected_item:
    ...
    delete_student(student_id, database)
...
def create_download_ui(tab, logged_in_user) -> None:
    ...
    term_combobox = ttk.Combobox(search_input_labelframe, textvariable=terms_var, values=terms, state='readonly', bootstyle='SUCCESS')
    ...
    search_button = ttk.Button(search_input_labelframe, text='Search', command=lambda: show_data_download(search_entry.get(), dat_var.get(), terms_var.get(), result_tree, searched_name_label))
    ...
    method_of_payment_entry = ttk.Combobox(input_labelframe, textvariable=method_of_payment_var, values=method_of_payment, state='readonly', bootstyle='SUCCESS')
    ...
    save_button = ttk.Button(input_labelframe, text='Update Record (Edit)', command=lambda: update_student_info_downloadui(student_id_input_entry, amount_expected_entry, amount_paid_entry, balance_entry, dop_entry, method_of_payment_var, transaction_entry, dat_var, terms_var, result_tree, search_entry), bootstyle=('SUCCESS', 'OUTLINE'))
    save_button.grid(row=2, column=3, padx=12, pady=5, sticky='ew')

    add_button = ttk.Button(input_labelframe, text='Add Record', command=lambda: add_student_details(student_id_input_entry.get(), amount_expected_entry.get(), amount_paid_entry.get(), balance_entry.get(), dop_entry.entry.get(), method_of_payment_var.get(), transaction_entry.get(), dat_var.get(), terms_var.get()), bootstyle=('WARNING', 'OUTLINE'))
    add_button.grid(row=2, column=4, padx=12, pady=5, sticky='ew')

    delete_button = ttk.Button(input_labelframe, text='Delete Record', command=lambda: delete_student_info_download(result_tree, dat_var), bootstyle=('DANGER', 'OUTLINE'))
    ...
    single_button = ttk.Button(download_labelframe, text='Single Download', bootstyle=('SUCCESS', 'OUTLINE'), command=lambda: create_student_pdf(
        id_download_entry.get(), student_id_download_entry.get(), dat_var.get(), terms_var.get(), logged_in_user))
    ...
    ToolTip(multiple_button, 'Ensure you have select the grade, year and database required', bootstyle=('INFO', 'INVERSE'))

    upload_button = ttk.Button(download_labelframe, text='Upload', bootstyle=('SUCCESS', 'OUTLINE'), command=still_in_development)
    upload_button.grid(row=1, column=6, columnspan=2, padx=6, pady=5, sticky='ew')

    # Function to handle the selection of an item for editing
    def on_edit_selected2(event):
...
def update_student_info_downloadui(student_id_entry, amount_expected_entry, amount_paid_entry, balance_entry, dop_entry, method_of_payment_var, transaction_entry, dat_var, terms_var, result_tree, search_entry):
    selected_item = result_tree.selection()
    if not selected_item:
        Messagebox.show_error('No student selected for editing','Error')
        return
    ...
    amount_expected = amount_expected_entry.get()
    amount_paid = amount_paid_entry.get()
    balance = balance_entry.get()
    dop = dop_entry.entry.get()
    method_of_payment = method_of_payment_var.get()
    transaction = transaction_entry.get()
    database = dat_var.get()
    term = terms_var.get()

    if not all([student_id, amount_expected, amount_paid, balance, dop, method_of_payment, transaction, database, term]):
        Messagebox.show_error('All fields must be filled out to update a student.','Error')
        return

    update_student_download(first_id, student_id, amount_expected, amount_paid, balance, dop, method_of_payment, transaction, database, term)
    show_data_download(search_entry, dat_var, terms_var, result_tree)
    Messagebox.show_info('Student information updated successfully', 'Success')


def delete_student_info_download(result_tree, dat_var):
    selected_item = result_tree.selection()
    if not selected_item:
        Messagebox.show_error('No student selected for deletion','Error')
        return
    ...
    delete_student_download(student_id, database)
    result_tree.delete(selected_item)
    Messagebox.show_info('Student deleted successfully', 'Success')
