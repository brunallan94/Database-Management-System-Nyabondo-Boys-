...
from student import add_student, show_data, process_file, add_student_details, create_student_pdf, search_students, update_student, create_all_student_pdf
import pandas as pd
...
def create_main_page_ui(tab) -> None:
    ...
    database_choice_label = ttk.Label(database_used_frame, text='Database')
    ...
    show_button = ttk.Button(database_used_frame, text='Show', command=lambda: show_data(result_tree1, dat_var), bootstyle=('INFO', 'OUTLINE'))
    show_button.grid(row=0, column=4, padx=12, pady=5, sticky='ew')
